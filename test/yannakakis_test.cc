...
TEST_CASE("Tadpole query") {
  ...
  root_bag->relations.push_back(TrieNode::FromFile("data/tadpole_tail.txt", {"a", "e"}));

  root_bag->children.push_back(unique_ptr<BagNode>(new BagNode));
  root_bag->children[0]->relations.push_back(TrieNode::FromFile("data/tadpole_tail.txt", {"e", "f"}));
  ...
  REQUIRE(result->contains({1, 2, 3, 4, 100, 200}));
}
